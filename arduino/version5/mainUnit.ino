...
#define ARDUINO_AUTOREBOOT_MINUTES 10 //if no ack is received within this minutes arduino autoreboots
...
#define ANEMOMETER_SPEED_FACTOR 1.7 //if no specs available use this setting
#define ANEMOMETER_CIRCUMFERENCE_MTS 0.53957//0.4367 // circumference in meters from center cup of anemometer
#define ANEMOMETER_CYCLES_PER_LOOP 2 // typically cup anemometers count twice on a full loop
...
void captureAndSendPartialSample() {
  ...
  //angle = encoder.getAngle()*0.08;
  //int windAngle = angle;
    int windAngle = encoder.getAngle()*0.08;
...
void captureAndSendMinuteSample() {
  ...
  int lightLvl = 0;//lightMeter.readLightLevel();
